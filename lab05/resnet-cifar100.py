...
def logger(engine, model, evaluator, loader, pbar):
    ...
    pbar.log_message("Test Results - Avg accuracy: {:.2f}".format(avg_accuracy))
...
if __name__ == '__main__':
    ...
    lr = options.lr
    print(lr)
    device = 'cpu' if options.device is None else torch.device('cuda:{}'.format(options.device))
    ...
    trainer = create_supervised_trainer(model=model, optimizer=optimizer, loss_fn=criterion, device=device)
